...
def main():
    ...
    ap.add_argument("--yaws", default="0", help="camera rotations of the perspective view to export, e.g. 0,180")
    ...
    if args.remove:
        ...
        yaws = [int(y) for y in args.yaws.split(",") if y.strip()]

        def export_image(src, name):
            ...
            return f"stimuli/mazebench/images/{name}"

        items = []
        for pair in pairs:
            for r in pair:
                images = {}
                src_images = r.get("images") or {}
                for view in views:
                    if view == "perspective":
                        yaw_images = src_images.get("yaws") or {}
                        for y in yaws:
                            src = yaw_images.get(str(y)) or (src_images.get("perspective") if y == 0 else None)
                            if not src:
                                continue
                            name = f"{r['id']}-perspective.png" if y == 0 else f"{r['id']}-perspective-y{y}.png"
                            path = export_image(src, name)
                            images.setdefault("yaws", {})[str(y)] = path
                            if y == 0:
                                images["perspective"] = path
                    else:
                        src = src_images.get(view)
                        if src:
                            images[view] = export_image(src, f"{r['id']}-{view}.png")
                ascii_text = ""
        ...
        sets[args.set] = {"label": args.label or args.set, "views": views, "yaws": yaws, "items": items}
